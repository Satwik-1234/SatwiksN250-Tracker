      ...
      {accessories.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {accessories.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-white rounded-3xl border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all group flex flex-col justify-between overflow-hidden relative">
                  ...
                  <div className="w-full h-full rounded-2xl overflow-hidden relative group/img cursor-pointer bg-slate-50 border border-slate-100" onClick={() => {
                    ...
                    {item.photoUrl ? (
                      docType === 'IMAGE' ? (
                        ...
                        <div className="w-full h-full bg-red-50/30 flex flex-col items-center justify-center">
                          ...
                          <span className="text-[10px] font-bold text-red-600 uppercase tracking-widest">PDF Invoice</span>
                        </div>
                      ) : (
                        <div className="w-full h-full bg-emerald-50/30 flex flex-col items-center justify-center">
                          ...
                          <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">HTML Invoice</span>
                        </div>
                      )
                    ) : (
                    ...
                    {item.photoUrl && (
                      <div className="absolute inset-0 bg-slate-900/5 opacity-0 group-hover/img:opacity-100 transition-opacity z-20 flex items-center justify-center pointer-events-none">
                        <div className="bg-white/95 backdrop-blur-sm text-slate-900 px-4 py-2 rounded-full font-semibold text-xs shadow-xl transform translate-y-2 group-hover/img:translate-y-0 transition-transform duration-300 flex items-center gap-1.5">
                ...
                <div className="p-5 pt-2 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-1">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-[10px] uppercase tracking-widest font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">{item.category}</span>
                        {item.brand && <span className="w-1 h-1 rounded-full bg-slate-200"></span>}
                        {item.brand && <span className="text-[10px] uppercase tracking-widest font-bold text-slate-400">{item.brand}</span>}
                      </div>
                  ...
                  <div className="flex items-center text-[11px] text-slate-400 font-medium mt-auto pt-4">
